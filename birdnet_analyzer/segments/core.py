    ...
    *,
    ...
    Args:
        ...
        threads (int, optional): Number of CPU threads to use for parallel processing.
            Defaults to 1.
        collection_mode (Literal["random", "confidence", "balanced"], optional): Mode for collecting segments.
    ...
    file_list = parse_files(result_collection, max_segments=max_segments, collection_mode=collection_mode, n_bins=n_bins, min_conf=min_conf, max_conf=max_conf)
    ...
    if threads < 2:
        for i, (path, segments) in enumerate(file_list, start=1):
            ...
            result_list.append(extract_segments(path, output, seg_length, segments, audio_speed=audio_speed))
    else:

        def wrapper(entry):
            return extract_segments(entry[0], output, seg_length, entry[1], audio_speed=audio_speed)
